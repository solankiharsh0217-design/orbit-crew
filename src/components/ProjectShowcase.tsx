...
export default function ProjectShowcase({ limit }: { limit?: number }) {
  ...
  return (
    <div className="w-full">
          ...
          return (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm transition-all duration-200 cursor-pointer ${
                isActive
                  ? "bg-[#2377F6] text-white font-medium shadow-md"
      ...
      {/* Grid of Projects with Lightweight 60fps GPU Hardware Acceleration */}
      <div className="grid grid-cols-2 gap-8 max-[1080px]:grid-cols-1">
        {displayedProjects.map((project) => (
          <div
            key={project.id}
            className="group relative flex flex-col rounded-2xl bg-[#1c1c1c] border border-white/10 overflow-hidden shadow-lg transition-transform duration-200 ease-out hover:-translate-y-1 hover:border-[#2377F6]/50 transform-gpu will-change-transform"
          >
            ...
            <div className="relative aspect-[16/9] w-full overflow-hidden bg-black flex-shrink-0">
              <img
                ...
                loading="lazy"
                decoding="async"
                className={`w-full h-full object-cover ${project.objectPosition || "object-top"} transition-transform duration-300 group-hover:scale-102 select-none transform-gpu`}
              />
              
              {/* Highlight Metric Overlay Badge (Lightweight Solid Glass Base) */}
              <div className="absolute bottom-4 right-4 bg-[#0a0a0a]/95 border border-white/15 px-4 py-2 rounded-xl text-right shadow-xl pointer-events-none">
